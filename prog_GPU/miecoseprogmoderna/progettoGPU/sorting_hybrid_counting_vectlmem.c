...
int main(int argc,char** argv){
	...
	double time_local_merge = mergeSortLocal(h_Sort,nels,lws);
	//printarr(h_Sort,nels);
	verify(h_Sort,nels);
	...
	printf("sort_local_host: %d int in %gms: %g GB/s %g GE/s\n",
		nels, time_local_merge, sort_local_bw_gbs, (nels)/1.0e6/runtime_sort_ms);
	...
	printf("total_sort: %d int in %gms: %g GB/s %g GE/s\n",
		nels, runtime_sort_ms+time_local_merge, (sort_bw_gbs + sort_local_bw_gbs)/2, (nels)/1.0e6/runtime_sort_ms);
