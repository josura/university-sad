	...
	cl_event init_evt)
{
	...
	const size_t lws[] = { lws_cli };
	const size_t gws[] = { round_mul_up(nquarts, lws[0]) };
	...
	err = clSetKernelArg(vecsmooth_k, i++, (lws[0])*sizeof(cl_int)*2, NULL);
	ocl_check(err, "set vecsmooth arg", i-1);
...
void verify(const int *vsmooth, int nels)
{
	for (int i = 0; i < nels; ++i) {
		...
		if (vsmooth[i] != target) {
			fprintf(stderr, "mismatch @ %d : %d %d %d %d %d %d %d %d %d %d diverso da  %d\n",
				i, vsmooth[i],vsmooth[i+1],vsmooth[i+2],vsmooth[i+3],vsmooth[i+4],vsmooth[i+5],vsmooth[i+6],vsmooth[i+7],vsmooth[i+8],vsmooth[i+9],target);
			exit(3);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	cl_kernel vecsmooth_k = clCreateKernel(prog, "vecsmooth_vect_lmem", &err);
